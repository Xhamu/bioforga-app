...
<x-filament::widget>
    @php
        ...
        $hexSoft = $hex ? $mix($hex, [255, 255, 255], 0.92) : null;
        $hexRing = $hex ?: '#E5E7EB';
        ...
        // Clases y estilos precompuestos
        $bgBase = 'pointer-events-none absolute inset-0 opacity-60 dark:opacity-40';
        $bgClass = $hex
            ? $bgBase
            : $bgBase . ' bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-900/60';
        $bgStyle = $hex
            ? "--grad-a: {$hexSoft}; --grad-b: {$hex}; background:
    ...
    <x-filament::card
        class="rounded-2xl shadow-sm border relative overflow-hidden border-gray-200/70 dark:border-white/10 p-6 md:p-8">

        {{-- Fondo dinámico --}}
        <div aria-hidden="true" class="{{ $bgClass }}"
            @if ($bgStyle) style="{{ $bgStyle }}" @endif></div>
            ...
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    @if ($isActive)
                        @if ($hex)
                            <span
                                class="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm ring-1 ring-inset backdrop-blur-sm"
                                style="background: {{ $hexSoft }}; color: {{ $hexText }}; border-color: {{ $hexRing }};">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" wire:poll.1s>
                    <div class="col-span-2">
                        <div class="flex items-center gap-3 p-4 rounded-xl ring-1 ring-inset backdrop-blur"
                            style="background: {{ $hexSoft }}33; border-color: {{ $hexRing }};">
                    ...
                    <div>
                        <div class="p-4 rounded-xl ring-1 ring-stone-900" style="background: white; border-color: red;">
                            <div class="flex items-center gap-3">
